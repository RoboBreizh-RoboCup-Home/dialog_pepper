...
class CommandProcessor(object):
    ...
    def predict(self, lines: List[str]) -> str:
        ...
        print('------------------------------------------------------')
        rospy.loginfo(B+"[Robobreizh - Dialog Predict] Total inference time: " +
                      W + str(time.time() - start))
    ...
    def write_readable_outputs(self, slot_preds_list, intent_token_preds_list, referee_preds_list):
        ...
        with open(self.output_file, "a", encoding="utf-8") as f:
            if 'B-referee' in referee_preds_list:
                ...
                f.write(line.strip()+'\n')
            rospy.loginfo(B+"[Robobreizh - Dialog Predict] Parsed token: "+W+line)
...
if __name__ == "__main__":
    ...
    res = inference.predict("Contact Skyler at the couch then greet him please".split())
    print(res)
